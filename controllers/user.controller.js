...
router.get('/get-user', async (req, res) => {
  try {
    ...
    const sql = `SELECT * FROM sitelok WHERE Username = ?`;
...
router.post('/signup', async (req, res) => {
  try {
    ...
    const values = [Username, Email, hashedPassphrase, Name, 'HUNTER'];
  ...
  try {
    ...
    if (!NewPassword || !Username || !Email || !ResetToken) {
      return res.status(400).json({
        message: 'New password, username, email, and reset token are required',
      });
    }
    ...
    db.query(sql, async (error, userArray) => {
      ...
      const updateSql = 'UPDATE sitelok SET Passphrase = ?, ResetToken = NULL WHERE Username = ?';
